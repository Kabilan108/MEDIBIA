...
def getsymptomList(userText):
    """Use ChatGPT to get a comma-separated list of symptoms from the user"""
    ...
    symptoms = response['choices'][0]['message']['content'].split(', ')
    return symptoms

def encode_symptoms(symptoms):
    symptoms = [1 if symptom in symptoms else 0 for symptom in labels]
